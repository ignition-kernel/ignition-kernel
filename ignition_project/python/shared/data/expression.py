...
from shared.data.types.enum import MetaEnum, Enum
			...
			# Otherwise the token is just a part of the formula
				...
				if token == '(' and output and (output[-1][0] == tokenize.NAME or output[-1] == (tokenize.OP, '.')):
					if output[-1] == (tokenize.OP, '.'):
						...
						opstack.extend([output.pop(),output.pop(),output.pop()])
...
class Expression(object):
	...
	def __init__(self, expression):
		self._source_expression = expression
...
#			print 'Postfix', postfixStack
		...
		for tokenType,token in postfixStack:
			...
			if tokenType == tokenize.OP:
				...
				if token in whitelisted_modules:
					...
					# NO
					# math.sin(x) --> ((1, 'x'), (1, 'math'), (1, 'sin'), (51, '.'))
					# solve x, then apply math.sin on it
	...
	def __call__(self, *args, **kwargs):
		...
		if kwargs:
			self._arguments[:] = [kwargs.get(field, args[i] if len(args) > i else None) 
	...
	def __repr__(self):
		return 'Expression(%r)(%s)' % (self._source_expression, ','.join(self._fields))
