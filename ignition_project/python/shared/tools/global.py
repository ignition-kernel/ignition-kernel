...
ExtraMetaExtraGlobal_HOLDING_THREAD_NAME = 'ExtraGlobal-Cache'



def get_extraglobal():

	sleep(0.05*random.random()+0.01)
	...
	_recover_holding_threads = findThreads(ExtraMetaExtraGlobal_HOLDING_THREAD_NAME)
	...
	MetaExtraGlobal = getFromThreadScope(_recover_holding_threads[0], 'MetaExtraGlobal')
	
	shared.tools.logging.Logger().trace('get init: %r init: %r' % (MetaExtraGlobal, MetaExtraGlobal._initialized))
	assert MetaExtraGlobal._initialized is not (None or False), 'ExtraGlobal not yet initialized!'
	...
	return ExtraMetaExtraGlobal, MetaExtraGlobal, ExtraGlobal



try:
	ExtraMetaExtraGlobal, MetaExtraGlobal, ExtraGlobal = get_extraglobal()

# ExtraGlobal not initialized yet
	...
	class ExtraMetaExtraGlobal(type):
		...
		"""
		HOLDING_THREAD_NAME = ExtraMetaExtraGlobal_HOLDING_THREAD_NAME
	...
	#		assert len(cache_threads) <= 1, "The ExtraGlobal-Cache thread has been spun up more than once! Only one should be alive: %r" % cache_threads
		...
		def spawn_holding_thread(meg_cls, MEG_Reference):
			...
			@async(0.01, meg_cls.HOLDING_THREAD_NAME)
	...
	class MetaExtraGlobal(type):
		...
		def __new__(cls, clsname, bases, attrs):
			...
			if cls._initialized is (None or False):
		...
		def __setattr__(cls, key, value):
			...
			elif cls._initialized is (None or False):
		...
		def spawn_cache_monitor(cls):
			...
			# Spin up the thread after a very short delay to allow final assignments to establish
			@async(0.01, cls.CLEANUP_THREAD_NAME)
			def monitor(cls=cls):
	...
	try:
		...
		for retry in range(5):
			...
			try:
				...
				shared.tools.logging.Logger().trace(error)
				continue
